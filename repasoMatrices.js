...
/**
 * La semana que mas se gasto y el dia que mas se gasto (lunes o martes ....)
 */
...
const mayorGastoMensual =(gastos3)=>{

        let sumaSemanas = [];


        for (let i = 0; i < gastos3.length; i++) {                
            let totalSemana = 0; 
            for (let j = 0; j < gastos3[i].length; j++) {
                totalSemana += gastos3[i][j];        
            }
            sumaSemanas.push(totalSemana)
    ...
    // sumaSemanas=mayorGastoMensual(gastos3);
    // console.log(sumaSemanas);



const mayorGastoDiario =(gastos3)=>{

        let sumaDias = [];

        for (let i = 0; i < 7; i++) {                      
            let totalDiario = 0; 
            for (let j = 0; j < gastos3.length; j++) {
                totalDiario += gastos3[j][i];
            }      
            
            sumaDias.push(totalDiario);
        }
    ...
    // sumaDias=mayorGastoDiario(gastos3);
    // console.log(sumaDias);
      


    const calcularMayorGastos = (gastos) => {
        const totalSemanas = [];
        ...
        for (let i = 0; i < gastos.length; i++) {
          ...
          for (let j = 0; j < gastos[i].length; j++) {
            ...
            totalDias[j] += gastos[i][j];
          }
      
          totalSemanas.push(totalSemana);
        }
      
        return { totalSemanas, totalDias };
      };
      
      let total = calcularMayorGastos(gastos);
      console.log(total);
